...
const MONEY_PATTERNS: RegExp[] = [
  /투자|수익|부업|재테크|코인|비트코인|리딩방|급등주|무료\s?상담/i,
  /crypto|profit|investment|forex|trading|earn\s?money|passive\s?income/i
];
...
export function detectSpam(rawText: string, context: SpamDetectionContext = {}): SpamDetectionResult {
  ...
  if (matchesAny(text, MONEY_PATTERNS)) {
    score += 3;
    reasons.push("money-promo");
  }

  if (matchesAny(text, PROMO_PATTERNS)) {
    score += 2;
    reasons.push("promotion");
  }

  if (matchesAny(text, CALL_TO_ACTION_PATTERNS)) {
    score += 1;
